#!/usr/bin/env Rscript
#Falta escrever a parte do HISAT2
...
option_list <- list(
    ...
    make_option(c("-M", "--mappingAlgorithm"), type="character", default="STAR",
                help="Mapping algorithm to use, supported types are 'HISAT2' and 'STAR' [default %default]",
...
if(!(opt$mappingAlgorithm %in% c("STAR", "HISAT2"))){
    stop("Mapping algorithm parameter must be one of 'STAR' or 'HISAT2'")
}

if(is.na(opt$mappingFolder)){
    if(casefold(opt$mappingAlgorithm) == "STAR")
        opt$mappingFolder = "02-MappedReads_STAR"
    else if(casefold(opt$mappingAlgorithm) == "HISAT2")
        opt$mappingFolder = "02-MappedReads_HISAT2"
    else
...
if (casefold(opt$mappingAlgorithm, upper = FALSE) == "STAR"){
...
}else if(casefold(opt$mappingAlgorithm, upper = FALSE) == "HISAT2"){
    hisat_mapping <- mclapply(mapping, function(index){
        try({
            system(paste("HISAT2"
                         ))
                         })
