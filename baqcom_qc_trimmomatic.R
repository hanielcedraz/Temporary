...
option_list <- list(
    ...
    make_option(c("-c", "--column"), type="character", default="SAMPLE_ID",
                ...
                dest="samplesColumn"),
    make_option(c("-l", "--fastqc"), action="store_true", default=FALSE,
                help="Use this option if you ould like to run fastqc analysis.  [default %default]",
                dest="fastqc"),
    ...
    make_option(c("-o", "--output"), type = "character", default = "01-CleanedReads",
                help = "Directory where the output files is stored [default %default]",
                dest = "output"),
...
opt <- parse_args(OptionParser(option_list = option_list, description =  paste('Authors: OLIVEIRA, H.C. & CANTAO, M.E.', 
                                                                               'Version: 0.2.3', 'E-mail: hanielcedraz@gmail.com', sep = "\n", collapse = '\n')))
...
if(length(samples[,1]) > 1){
    ...
    for (i in as.character(samples[,1])) {
        report_sample[i] <- read.table(paste(paste0(reports,'/', report_folder), '/', i, '_', 'statsSummaryFile.txt', sep = ""), header = F, as.is = T, sep = ':', row.names = 1);
    ...
    write.table(report_final, file = paste0(reports,'/','qc_report_trimmomatic.txt'), sep = "\t", row.names = FALSE, col.names = TRUE, quote = F)
}else{
    report_sample <- read.table(paste(paste0(reports,'/', report_folder), '/', samples[,1], '_', 'statsSummaryFile.txt', sep = ""), header = F, as.is = T, sep = ':', row.names = 1);
    trans_report <- t(report_sample); 
    colnames(trans_report) <- c('Input_Read_Pairs', 'Pairs_Reads', 'Pairs_Read_Percent', 'Forward_Only_Surviving_Reads', 'Forward_Only_Surviving_Read_Percent', 'Reverse_Only_Surviving_Reads', 'Reverse_Only_Surviving_Read_Percent', 'Dropped_Reads', 'Dropped_Read_Percent')
    report_final <- data.frame(Samples = samples[,1], trans_report);
    write.table(report_final, file = paste0(reports,'/','qc_report_trimmomatic.txt'), sep = "\t", row.names = FALSE, col.names = TRUE, quote = F)
}

system2('cat', paste0(reports,'/','qc_report_trimmomatic.txt'))
