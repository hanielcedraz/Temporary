...
option_list <- list(
    ...
    make_option(c("-c", "--column"), type = "character", default = "SAMPLE_ID",
                ...
                dest = "samplesColumn"),    
    make_option(c("-i", "--inputFolder"), type = "character", default = "02-MappedReads_HISAT2",
                help = "Directory where the sequence data is stored [default %default]",
                dest = "inputFolder"),    
    make_option(c('-E', '--countFolder'), type = 'character', default = '04-GeneCounts_HTSeq',
...
countingList <- function(samples, reads_folder, column){
    ...
    for (i in 1:nrow(samples)) {
        bamfiles <- dir(path = file.path(reads_folder), recursive = TRUE, pattern = ".bam$", full.names = TRUE)
        count <- lapply(c("_sam_sorted_pos"), grep, x = bamfiles, value = TRUE)
...
if (opt$multiqc) {
    if (file.exists(paste0(report_02,'/',fastqcafter)) || file.exists(paste0(report_02,'/',fastqcbefore)) || file.exists(paste0(report_02,'/', multiqc_data))) {
        ...
        system2('multiqc', c(opt$countsFolder, '-o', reportsall, '-f'))
